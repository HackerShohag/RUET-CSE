...
SELECT Movie_Name, IMDB_Rating from Movie ORDER BY IMDB_Rating DESC, Movie_Name DESC;
...
SELECT AVG(IMDB_Rating) FROM Movie WHERE Genre='Drama';

-- Query 1.5: Find the total number of movies in each genre.
SELECT COUNT(Movie_ID), Genre FROM Movie GROUP BY Genre;
...
SELECT AVG(No_Of_Films) FROM Director;
...
SELECT Movie_Name, Genre, IMDB_Rating, Director.Director_Name from Movie 
JOIN Director ON Director.Person_ID = Movie.Director_ID 
WHERE Director.Director_Name IN('Satyajit Ray', 'Zahir Raihan', 'Rituparno Ghosh', 'Kaushik Ganguly');

-- Query 2.3: Find the name and current age of all the directors and actors.
SELECT Director_Name As Name , YEAR(NOW()) - Birth_Year AS Age from Director
UNION SELECT Actor_Name as Name, YEAR(NOW()) - Birth_Year AS Age from Actor;
...
SELECT Actor_Name As Name , YEAR(NOW()) - Actor.Birth_Year AS Age,
Actor.No_Of_Films as AsActor,
Director.No_Of_Films as AsDirector from Actor
...
SELECT Director_Name As Name, COUNT(Movie.Movie_ID) as Movies from Director
JOIN Movie ON Movie.Director_ID = Director.Person_ID GROUP BY Director.Director_Name;